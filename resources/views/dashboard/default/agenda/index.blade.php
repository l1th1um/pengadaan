                        ...
                        <table class="table table-striped responsive-utilities jambo_table" id="announcement_list">
                            ...
                            <tbody>
                                @foreach($data as $row)
                                    <tr>
                                        <td> {{ $row->title }}</td>
                                        <td>{{ localeDate($row->agenda_date)}}</td>
                                        <td class="center">
                                            <a href="{{route('dashboard.agenda.edit', $row->id)}}">
                                                <img src="{{Theme::url('images/edit.png')}}">
                                            </a>
                                            <a href="{{route('dashboard.agenda.destroy', $row->id)}}" data-method="delete" data-token="{{csrf_token()}}" data-confirm="{{ trans('common.are_you_sure') }}" style="padding-left : 10px">
